...
final class Table_creator{

    private $req_wp_version = '4.0';
    public $plugin_pages = array('table-creator-all', 'create-table-page', 'attc-support', 'attc-export', 'attc-import');
    ...
    public function init() {
        ...
        if ( is_admin() ) {
            ...
            //@TODO: uncomment when the website is ready with full docs and faqs list.
    ...
    public function add_plugin_action_link(array $links) {
        ...
        $links[] = sprintf( '<a href="%s" title="%s">%s</a>', 'admin.php?page=table-creator-new', 'Add New', __( 'Add New', ATTC_TEXTDOMAIN ) );
        $links[] = sprintf( '<a href="%s" title="%s">%s</a>', 'admin.php?page=table-creator-all', 'View All', __( 'View All', ATTC_TEXTDOMAIN ) );
        return $links;
    ...
    public function add_plugin_row_meta_link(array $links, $file)
    {
        if ( ATTC_BASE === $file ) {
            ...
            //@TODO: enable support link only for pro users and when documentation is complete
            //$links[] = '<a href="https://aazztech.com/support/">' . __( 'Get Support', ATTC_TEXTDOMAIN ) . '</a>';
            $links[] = '<a href="https://aazztech.com/donate/" title="' . esc_attr__( 'Support Table Creator with your donation!', ATTC_TEXTDOMAIN ) . '"><strong>' . __( 'Donate', ATTC_TEXTDOMAIN ) . '</strong></a>';
        }
        return $links;
    }
