...
class ATTC_controller {
    ...
    private $default_pages = array('table-creator-all', 'create-table-page', 'attc-import', 'attc-export');
    ...
    public function register_shortocde($atts, $content = null)
    {
        ...
        wp_enqueue_style('attc-public-style');
    ...
    public function update_table() {
        if (!empty($_POST['attc-create-table']) && !empty($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'], 'attc_update_table')) {
            $t = !empty($_POST['table']) ? $_POST['table'] : array();
            $ID = !empty($_GET['table']) ? absint($_GET['table']) : null;
            $name = !empty($t['name']) ? sanitize_text_field($t['name']) : 'No Name';
            $description = !empty($t['description']) ? sanitize_textarea_field($t['description']) : '';
            $column = !empty($t['column']) ? absint($t['column']) : 0;
            $rows = !empty($t['row']) ? absint($t['row']) : 0;
            $content = !empty($_POST['table_values']) ? $_POST['table_values'] : array();
            $success = $this->db->update( $ID, $name, $description, wp_get_current_user()->display_name, $rows, $column, '', 0, $content );
            if($success) {
    ...
    public function create_table(){
        if ( $this->is_create_page() && $this->is_valid_form_data() ){
     ...
     * @return bool
     */
    private function is_valid_form_data(){
    ...
    {
        return (!empty($_GET['page']) && 'table-creator-all' == $_GET['page'] && !isset($_GET['action'])) ? true : false;
    }
    ...
    public function show_admin_menu(){
        ...
        add_menu_page(
            ...
            __('Table Creator', ATTC_TEXTDOMAIN),
            'manage_options',
            'table-creator-all',
        ...
        add_submenu_page('table-creator-all',
            __('All Tables', ATTC_TEXTDOMAIN),
            __('All Tables', ATTC_TEXTDOMAIN),
            'manage_options',
            'table-creator-all',
        ...
        //@TODO: uncomment this we start giving premium support for users
        //add_submenu_page('table-creator-all',
        //    __('Get Support', ATTC_TEXTDOMAIN),
        //    __('Get Support', ATTC_TEXTDOMAIN),
        //    'manage_options',
        //    'attc-support',
        //    array($this, 'show_support_view')
        //);
    }
    ...
    /**
     *Show table support view
     */
    public function show_support_view()
    {
        global $ATTC;
        $ATTC->loadView('view-support');
    }
    ...
    public function export_table() {
        ...
        $tables = ( ! empty( $export['tables_list'] ) ) ? explode( ',', $export['tables_list'] ) : $export['tables']; //  get tbl id(s)
            ...
            $export_data = apply_filters( 'table-creator_export_data', $export_data, $table, $export['format'], $export['csv_delimiter'] );
