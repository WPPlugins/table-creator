(function ($) {
    ...
    attc.table = {
        ...
        current_rows_length : parseInt($("#attc_table_admin tbody tr").length, 10),
        current_column_length : $('#attc_table_admin tbody tr:last td').length - 2, // get number length except first two empty tds
        ...
        reindex: function () {
            $('#attc_table_admin tbody tr').each(function (rowIDx) {
            ...
            $('#attc_table_admin thead tr th').not('.first_th').each(function (idx) {
      ...
      confirm_delete: function () {
        ...
        create: function () {
            ...
            if (!isNaN(colsTobeAdded)) {
                attc.table.current_column_length = $('#attc_table_admin tbody tr:last td').length - 2;
    ...
    attc.rows = {
        delete: function () {
            $checked_rows = $('.row_check:checked');
        ...
        create: function () {
            ...
            var curRowLen = $('#attc_table_admin tbody tr:last td').length - 2;
            if (!isNaN(rowTobeAdded)) {
                ...
                $('#attc_table_admin tbody').append(new_row_html);// append new row markup to the dom
    ...
    // Helper object
    attc.helper = {
        ajax_handler: function (ElementToShowLoadingIconAfter, ActionName, arg, CallBackHandler) {
            var data;
            ...
            var n = data.search(attc_obj.nonceAction);
            if (n < 0) {
                data = data + "&_wpnonce=" + attc_obj.nonce;
            }
    ...
    $( '#tables-export-select-all' ).on('click', function () {
        $( '#tables-export option' ).prop('selected', this.checked)
    });
     ...
     *
     */
    $('#attc-table-setting, #attc-table-setting-express').on('click', function (e) {
        e.preventDefault();
        var form = $("#attc_setting_form");
        var formData = form.serialize() + '&id=' + form.data('id');
        $("#successResult").remove();
        var $spiner_location = $(this);


        attc.helper.ajax_handler($spiner_location, 'attc_setting_handler', formData, function (data) {
            if (data === 'success') {
                swal({
                    title: "Success",
                    text: "Data Saved",
                ...
                attc.helper.auto_cLose_message('Error: Something went wrong. <span id="attc_close_it">X</span>', 2000);
    ...
    $('.confirmation').on('click', function (e) {
        ...
        swal({
            ...
            function(){
                // $(this).context.search.replace('?', ' '); // get the query string from the link and then remove this '?'
            attc.helper.ajax_handler('', 'delete_attc_table', $this.context.search.replace('?', ' '), function (data) {
